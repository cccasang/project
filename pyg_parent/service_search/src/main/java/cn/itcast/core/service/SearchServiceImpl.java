...
public class SearchServiceImpl implements SearchService {
    ...
    public Map<String, Object> search(Map paramMap) {
        ...
        //1.根据关键字高亮分页查询
        Map<String, Object> resultMap = highlightPageSearch(paramMap);
        ...
        if (categoryName  == null || "".equals(categoryName)){
            //如果消费者没有在页面上点击分类,则默认根据分类集合中的第一个分类查询对应的品牌和规格集合
            if (categoryNameList != null && categoryNameList.size()>0){
               categoryName = categoryNameList.get(0);
                Map<String, Object> brandAndSpecList = findBrandAndSpecByCategoryName(categoryName);
                resultMap.putAll(brandAndSpecList);
            }
        }else{
            //如果消费者在页面上点击分类则根据分类名称查询对应的品牌和规格集合
            Map<String, Object> brandAndSpecList = findBrandAndSpecByCategoryName(categoryName);
            resultMap.putAll(brandAndSpecList);
     ...
     * @return
     */
    public Map<String,Object> highlightPageSearch(Map paramMap){
        //获取从页面传入的查询参数
        //获取查询关键字
        ...
        /**
         * contains 和 is 的区别
         * contains方法: 这个语句有点类似于sql语句中的like模糊查询功能,将关键字当成一个整体进行模糊查询
         * is 方法: 会根据当前使用的分词器对关键字进行切分词,然后根据切分出来的词再进行一个一个查询
         */
    ...
    /**
     * 分组分页查询,目的是 根据关键字获取分类名称,分组的目的是为了给获取到的分类名称去重
     * @param paramMap  查询页面传入的查询条件
     * @return
     */
